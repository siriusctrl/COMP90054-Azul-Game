...
class myPlayer(AdvancePlayer):
    ...
    def SelectMove(self, moves, game_state):
        ...
        try:
            ...
            # select move for the first turn as there is not much extra info minmax can provide
            # but by doing this can prevent timeout
            if self.turn == 1:
                best_score = float("-inf")
                best_move = None
                current_state: GameState = deepcopy(game_state)
                expected_score, _ = current_state.players[self.id].ScoreRound()
                expected_bonus = current_state.players[self.id].EndOfGameScore()

                for move in moves:
                    move_score = self.greedyScore(move, game_state, expected_score, expected_bonus)
    ...
    def greedyScore(self, move: (Move, int, TileGrab), game_state: GameState, neighbour_bonus: float, bonus: float):
        """
        ...
        new_bonus = bonus - my_state.EndOfGameScore()
        if self.curr_round <= self.ROUND_IGNORE_FINAL_BONUS:
            final_score += new_bonus * self.final_bonus_weight
        else:
            final_score += new_bonus
