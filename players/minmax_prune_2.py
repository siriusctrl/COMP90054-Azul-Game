...
class myPlayer(AdvancePlayer):
    ...
    def SelectMove(self, moves: [(Move, int, TileGrab)], game_state: GameState):
        ...
        for move in top_5_actions:
            ...
            move_score = self.MiniMax(move, game_state, depth, self.id, [expected_score, game_state])
    ...
    def MiniMax(self, move: (Move, int, TileGrab), game_state: GameState, depth: int, player_id: int, root_data: list):
        # current_state = deepcopy(game_state)
        # expected_score, _ = current_state.players[self.id].ScoreRound()
        ...
        if depth == 0:
            ...
            return self.ExpectScoreAfterMyMove(move, game_state, next_state, root_data)
    ...
    def ExpectScoreAfterMyMove(self, move: (Move, int, TileGrab), game_state: GameState, next_game_state: GameState, root_data: list):
        ...
        next_game_state_copy = deepcopy(next_game_state)
        my_state = next_game_state_copy.players[self.id]

        neighbour_bonus = root_data[0]
        # root_state = root_data[1]
